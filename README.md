...
Exposes a generic software upgrade as a native HA `update` entity. What gets detected and upgraded (version source, detection schedule, upgrade command) is **entirely configured on the go-odio-api server** — HA just reflects it.
...
| Entity | Description |
|--------|-------------|
| `update.odio_remote_[hostname]_software` | Generic software update — installed/latest version, install button (when allowed), live progress |
