...
1. **`OdioReceiverMediaPlayer`** — the Odio instance itself. Always available; state OFF while disconnected. PLAYING when any audio client is uncorked or any MPRIS player plays. Volume read = `hub.audio.volume`/`.muted` (default output sink, falling back to the `/audio/server` snapshot — never the client streams, which PipeWire pins at 1.0); write = `hub.audio.set_volume` (master). Mute via `hub.audio.set_muted(bool)` (pyodio compare+toggle — the server only toggles). Source select = default output via `AudioOutput.make_default()`.
...
4. **`OdioMPRISMediaPlayer`** — one per MPRIS app (deduped by app name; `helpers.extract_mpris_app_name` strips the `.instanceXXX` suffix pyodio's `app_name` keeps — it is load-bearing for unique_ids). Created dynamically via `hub.players.on_change` (ADDED); an app restart with a new bus_name **rebinds** the existing unavailable entity instead of creating one. `media_position` is the raw beacon (µs→s) + `media_position_updated_at` — HA extrapolates. `media_image_url` = `Player.cover_url` (server-side `/cover` proxy with cache-busting query) only when `art_url` is set. The MPRIS **TrackList** is mapped onto HA's browse/enqueue surfaces (HA has no queue state): `async_browse_media` lists `player.tracks` (children keyed by `track_id`, `file://` artwork dropped since only the `/cover` proxy can serve it), `async_play_media` does `go_to` on a browsed `track_id` and `add_track` otherwise (`ADD` appends, `NEXT`/`PLAY`/no-enqueue insert after `current_track`, `REPLACE` clears first), and `async_clear_playlist` loops `remove_track` — MPRIS has no bulk clear. `BROWSE_MEDIA`/`PLAY_MEDIA` need `tracklist_supported`, `MEDIA_ENQUEUE`/`CLEAR_PLAYLIST` need `can_edit_tracks`.
...
CI runs on Python 3.14 via `.github/workflows/ci.yml` (Home Assistant 2026.3+ requires Python 3.14). pyodio is pinned to a PyPI release in both `pyproject.toml` and `manifest.json` — bump the two together.
