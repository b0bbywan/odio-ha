...
class TestMPRISPlayMedia:
    ...
    async def test_stale_track_id_refreshes_then_goes_to_it(self):
        stale = "/com/spotify/track/ghi"
        entity, hub = _with_tracklist()
        hub.client.get_player_tracklist.return_value = TracklistState.from_dict(
            {**MOCK_TRACKLIST, "tracks": [*MOCK_TRACKLIST["tracks"], {"track_id": stale}]}
        )
        await entity.async_play_media(MediaType.MUSIC, stale)
        hub.client.get_player_tracklist.assert_awaited_once_with(SPOTIFY_BUS)
        hub.client.player_tracklist_goto.assert_awaited_once_with(SPOTIFY_BUS, stale)
        hub.client.player_tracklist_add.assert_not_awaited()

    async def test_stale_track_id_never_reaches_add_track(self):
        entity, hub = _with_tracklist()
        hub.client.get_player_tracklist.return_value = TracklistState.from_dict(MOCK_TRACKLIST)
        with pytest.raises(HomeAssistantError):
            await entity.async_play_media(MediaType.MUSIC, "/com/spotify/track/gone")
        hub.client.player_tracklist_goto.assert_not_awaited()
        hub.client.player_tracklist_add.assert_not_awaited()
