...
class OdioMPRISMediaPlayer(MappedEntityMixin, OdioEntity, MediaPlayerEntity):
    ...
    async def async_play_media(
        ...
        # A browsed media_id is a track_id — a D-Bus object path. URIs never
        # start with a bare "/", so an object path must never reach add_track.
        if media_id.startswith("/"):
            if not any(track.track_id == media_id for track in player.tracks):
                await player.refresh_tracklist()
            if not any(track.track_id == media_id for track in player.tracks):
                raise HomeAssistantError(
                    f"Track {media_id} is no longer in the tracklist"
                )
